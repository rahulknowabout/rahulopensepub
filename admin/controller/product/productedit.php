<?php //echo "<pre>";
//print_r($_REQUEST);$GLOBALS['dblink']
//echo "<hr/>";
//print_r($_FILES);die;
...
$postarray =array();
$postarray['product_id'] =$cleaned['draft_product_id'];
...
header("location: ../../view/product/list.php");
	...
	if(isset($cleaned['product_type']) && $cleaned['product_type']!="" && $cleaned['product_type']=='program') {
		$parent_product = singleProduct($cleaned['product_id']);
//echo "<pre>";
...
if (isset($_FILES['program_file_first']) && is_array($_FILES['program_file_first']) && isset($_FILES['program_file_first']['name']) && $_FILES['program_file_first']['name'] != "" && isset($_FILES['program_file_first']['size']) && $_FILES['program_file_first']['size']>0) {
   ...
   $programFileName =$parent_product['product_file'];
}
$postarray['product_reg_date'] = $cleaned['product_reg_date'];
$postarray['product_type'] = $cleaned['product_type'];
...
//print_r($_FILES);
//die;
if (isset($cleaned['product_id']) && $cleaned['product_id']!= "" && isset($cleaned['draft_product_id']) && $cleaned['draft_product_id']!= "" && $cleaned['product_id']!= $cleaned['draft_product_id'] ) {
	$drfatIosProduct = productIos($cleaned['draft_product_id']);
	if (isset($drfatIosProduct) && is_array($drfatIosProduct) && count($drfatIosProduct)>0 ){
		deleteIosProduct($cleaned['product_id']);
	}
	$drfatAndroidProduct = productAndroid($cleaned['draft_product_id']);
	if (isset($drfatAndroidProduct) && is_array($drfatAndroidProduct) && count($drfatAndroidProduct)>0 ){
		deleteAndroidProduct($cleaned['product_id']);
	}
	$drfatWindowsProduct = productwindows($cleaned['draft_product_id']);
	if (isset($drfatWindowsProduct) && is_array($drfatWindowsProduct) && count($drfatWindowsProduct)>0 ){
		deleteWindowsProduct($cleaned['product_id']);
	}
	$drfatLinuxProduct = productLinux($cleaned['draft_product_id']);
	if (isset($drfatLinuxProduct) && is_array($drfatLinuxProduct) && count($drfatLinuxProduct)>0 ){
		deleteLinuxProduct($cleaned['product_id']);
	}
	$drfatOsxProduct = productOsx($cleaned['draft_product_id']);
	if (isset($drfatOsxProduct) && is_array($drfatOsxProduct) && count($drfatOsxProduct)>0 ){
		deleteOsxProduct($cleaned['product_id']);
	}
	DeleteParentProduct($cleaned['product_id']);
}
