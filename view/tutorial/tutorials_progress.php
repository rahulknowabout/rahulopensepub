...
if (isset($_POST['AddFile']) && $_POST['AddFile'] == 'select') {
									$fileNameArray =explode("/",$_POST['exist_file']);
									$keyNext = array_search($_POST['provider_id'],$fileNameArray);
									$fileRealName = $fileNameArray[$keyNext+1];
$data_array['masterEpub']	= 	$_POST['exist_file'];
$data_array['drmType']	= 	'opens';
...
if(isset($_POST['ServiceType']) && $_POST['ServiceType'] == 'user') {
	$data_array['target']['type']	= 	'user';
	$data_array['target']['userId']	= 	$_POST['user_id'];
}else{
	$device_array = explode(",",$_POST['device_name']);
	$data_array['target']['type']	= 	'device';
	$data_array['target']['userId']	= 	$_POST['user_id'];
	$data_array['target']['deviceId']	= 	$device_array[1];
	//$data_array['target']['deviceIdType']	= 	$device_array[0];
	$data_array['target']['deviceIdType']	= 	'internal';
...
$result = curl_exec($ch);
//print_r($result);die;
 if(file_exists("../../storage/epub/".$fileRealName)) unlink("../../storage/epub/".$fileRealName);
$destination = "../../storage/epub/".$fileRealName;
$file = fopen($destination, "w+");
fputs($file, $result);
fclose($file);
$tutorialHistoryArray['tutorial_file_name']  = $fileRealName;
$tutorialHistoryArray['tutorial_file_type']  = 'opens';
...
header('location:tutorials-complete.php?success='.$fileRealName);
}
...
<ul class="nav nav-tabs">
    ...
    <li>
        <a data-toggle="tabdisable" href="#Password_DRM">
...
<?php if (isset($_POST['AddFile']) && $_POST['AddFile'] == 'myepub') { ?>
...
//echo "<pre>";
 ...
 //$url ="http://stackoverflow.com";
	...
	$args['file'] = new CurlFile("".$_FILES['myepub_file']['tmp_name']."");
	$args['fileName'] = "".$_FILES['myepub_file']['name']."";
	//print_r($args);die;
    $options = array(
        CURLOPT_URL => $url,
...
if(isset($returnArray['result']) && $returnArray['result'] == 'error' ) {
	...
	//ob_end_clean( );
	if (isset($_POST['AddFile']) && $_POST['AddFile'] == 'myepub') {
...
addTutorialHistory($tutorialHistoryArray);	
	...
	$string = '<script type="text/javascript">';
    $string .= 'window.location = "tutorials-complete.php?success='.$fileRealName.'"';
    $string .= '</script>';
